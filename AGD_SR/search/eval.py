...
def main():
    ...
    transforms_ = [ transforms.CenterCrop(1024),
                    transforms.ToTensor(),
...
def infer(model, test_loader):
    ...
    for i, batch in enumerate(test_loader):
        ...
        mean_out, std_out = stat_calc(real_A)
        real_A = Normalize(mean_out, std_out)(real_A)
        real_A = UnNormalize(mean_in, std_in)(real_A)
        if not config.real_measurement:
            ...
            save_image(real_A, os.path.join('output/input', img_name))
