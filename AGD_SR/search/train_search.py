...
from util_gan.norm import UnNormalize, Normalize, stat_calc
...
def main(pretrain=True):
    ...
    teacher_model.load_state_dict(torch.load(config.generator_A2B)['gen_state_dict'], strict=True)
    ...
    transforms_ = [ transforms.RandomCrop(4*config.image_height), #added
                    transforms.ToTensor(),
                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
        ...
        if epoch and not (epoch+1) % config.eval_epoch:
            ...
            with torch.no_grad():
                if pretrain == True:
                    ...
                    infer(epoch, model, test_loader, logger)
                    ...
                    flops = infer(epoch, model, test_loader, logger, finalize=True)
...
def infer(epoch, model, test_loader, logger, finalize=False):
    ...
    for i, batch in enumerate(test_loader):
        ...
        img = real_A.clone()
        mean_in, std_in = stat_calc(unorm(img).cuda())
        
        fake_B = model(real_A)
        mean_out, std_out = stat_calc(fake_B)
        fake_B = Normalize(mean_out, std_out)(fake_B)
        fake_B = UnNormalize(mean_in, std_in)(fake_B)
